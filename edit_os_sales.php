...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if (!empty($getid)) {
        $query = mysqli_query($con, "SELECT * FROM other_shop_sales WHERE oss_id='$getid'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $oss_date = $_POST['oss_date'];
    $amount = $_POST['amount'];


    $query = mysqli_query($con, "SELECT * FROM other_shop_sales WHERE oss_id='$oss_id'");
    $row = mysqli_fetch_assoc($query);

    if (mysqli_query($con, "UPDATE other_shop_sales SET other_shop_id='$other_shop_id',description='$description',quantity='$quantity', oss_date='$oss_date',amount='$amount' WHERE oss_id='$oss_id'")) {

        // insert access log in to data base
        $user_email = $_SESSION['email'];
        $user_id = getIdByEmail($user_email);
        insertActivityLog($user_id,'Updated Other Shop Sales ' . otherShopName($other_shop_id));
        ...
        echo '<script type="text/javascript">';
        echo "setTimeout(function () { window.open('other_shop_sales.php','_self')";
        echo '}, 1500);</script>';
                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <h3 class="card-title">Update Other Shop Sales Information</h3>

                        </div>


                        <form role="form" id="quickForm" method="post" action="edit_os_sales.php">
                            <div class="card-body">


                                <div class="form-group">
                                    <input type="hidden" name="oss_id" class="form-control" id="exampleInputOssId"
                                           value="<?php echo $row['oss_id'] ?>">
                                </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="date" name="oss_date" class="form-control" id="exampleInputDate"
                                                   value="<?php echo $row['oss_date'] ?>">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card-footer">
                                <button type="submit" id="update_sales" class=" btn btn-primary"><i class="fa fa-check-circle"></i> Update Sales</button>
                            </div>
                        </form>
