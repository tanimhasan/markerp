...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($tran_type == 'Deposit'){
        ...
        $updated_amount=$get_amount+$amount;
        mysqli_query($con,"UPDATE banking SET balance='$updated_amount' WHERE account_id='$account_id'");
    }else{
        $get_amount=getBankAmount($account_id);
        $minus_amount=$get_amount-$amount;
        mysqli_query($con,"UPDATE banking SET balance='$minus_amount' WHERE account_id='$account_id'");
    }


    $result = mysqli_query($con, "SELECT * FROM transaction WHERE account_id='$account_id' AND tran_date='$tran_date' AND amount='$amount' AND payment_method='$payment_method' AND reference='$reference' AND tran_type='$tran_type'");
    $num_rows = mysqli_num_rows($result);


    if ($num_rows > 0) {
        echo '<script type="text/javascript">';
        echo 'setTimeout(function () { swal.fire("ERROR!","Transaction already exists!","error");';
        echo '}, 500);</script>';
    } else {
        if (mysqli_query($con, "INSERT INTO transaction (`account_id`,`tran_date`,`amount`,`payment_method`,`reference`,`tran_type`) VALUE ('$account_id','$tran_date','$amount','$payment_method','$reference','$tran_type')")) {

            // insert access log in to data base
            $user_email = $_SESSION['email'];
            $user_id = getIdByEmail($user_email);
            insertActivityLog($user_id,'Added New Transaction');
            // End insert access log in to data base
            echo '<script type="text/javascript">';
            echo 'setTimeout(function () { swal.fire("Transaction Successfully Added!","Done!","success");';
            echo '}, 500);</script>';
            echo '<script type="text/javascript">';
...
<head>
    ...
    <title>Add Transaction Info</title>
                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <h3 class="card-title">Add Transaction Information</h3>

                        </div>


                        <form role="form" id="quickForm" method="post" action="add_transaction.php">
                            <div class="card-body">

                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="exampleInputWeightUnit">Select Account</label>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="date" name="tran_date" class="form-control" id="exampleInputAccountNumber" value="<?php echo date("Y-m-d")?>">
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="tran_type">
                                                <option disabled selected hidden>Select Transaction Type</option>
                            ...
                            <div class="card-footer">
                                ...
                                <button type="submit" id="add_transaction" class="btn btn-primary"><i class="fa fa-check-circle"></i> Add
                                    Transaction
                                </button>
                            </div>
                        </form>
